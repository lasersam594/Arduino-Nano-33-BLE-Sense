...
#define GR_COR   0   // These are board-specific offset nulls
#define GP_COR   0
#define GY_COR   0
  ...
  while (!IMU.gyroscopeAvailable()) {}
    {
      ...
      if (data1 == 1) {
        ...
        sprintf(buffer, "%8.2f", gr );
        Serial.print(buffer);
        if (verbose1 == 1) Serial.print( "  P: ");
        sprintf(buffer, "%8.2f", gp );
        Serial.print(buffer);
        if (verbose1 == 1) Serial.print( "  Y: ");
        sprintf(buffer, "%8.2f"  , gy );
        Serial.print(buffer);
        if (verbose1 == 1) Serial.print(" | ");
      }
...
/*
  analogWrite(LEDR, 255-(r/16)); // Removed due to useless behavior
  analogWrite(LEDG, 255-(g/16)); 
  analogWrite(LEDB, 255-(b/16)); 
*/
